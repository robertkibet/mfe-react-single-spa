...
####
####
#  Build step: Deploy Products to bucket
####
####
resource "google_cloudbuild_trigger" "products_deployer" {
  provider = google-beta
  name     = "products-deployer"
    ...
    step {
      ...
      args = [
        ...
        <<-EOF
                  ...
                  git clone https://robertkibet:${data.google_secret_manager_secret_version.git_login_token.secret_data}@github.com/robertkibet/mfe-react-single-spa.git
                  git fetch
...
output "build_trigger_data" {
  ...
  value       = "https://console.cloud.google.com/cloud-build/builds?query=trigger_id%3D%22${google_cloudbuild_trigger.products_deployer.trigger_id}%22"
}
